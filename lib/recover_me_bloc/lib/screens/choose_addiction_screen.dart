...
class _ChooseAddictionScreenState extends State<ChooseAddictionScreen> {
  ...
  // Save addiction to the database and proceed to the next screen
  Future<void> _saveAddiction() async {
    ...
    try {
      // Create an instance of DatabaseHelper directly
      final dbHelper = DatabaseHelper();
      final db = await dbHelper.database;

      await db.update(
        DatabaseHelper.userTable, // Update the 'user' table
        {DatabaseHelper.columnUserAddiction: _selectedAddiction}, // Set the addiction value
        where: '${DatabaseHelper.columnUserEmail} = ?', // Identify the user by email
        whereArgs: [widget.email],
      );

      // Navigate to the dashboard page after successful update
      Navigator.pushNamed(context, '/', arguments: {'selectedAddiction': _selectedAddiction});
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
      ...
      body: Container(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 20),
            ElevatedButton(
              onPressed: _saveAddiction,  // Call _saveAddiction to save the selected addiction
